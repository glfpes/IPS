...
namespace vlc {
	...
	std::vector<vlc::Transmitter>* Processor::OpenFFT(cv::Mat Image, CameraInfo* Camera) {
		...
		for (unsigned i = 0; i < Contours->size(); ++i) {
			...
			Light.PositionInImage = cv::Point2d((*Centers)[i].x - GrayImage.size[1] / 2, GrayImage.size[0] / 2 - (*Centers)[i].y);
			Light.ID = ID;
	...
	void Processor::PairLights(std::vector<vlc::Transmitter>* Lights, RoomInfo * Room)
	{
		char cstr[1024];
		std::string lightStr = "";
		std::ofstream out_delete;
		out_delete.open("pairs.txt", std::ios::trunc);
		//system("rm pairs.txt");
